...
There are only \(b\) Weedles (failure) and \(w\) Pikachus (success) in Viridian Forest. We encounter \(N\) Pokemon in the forest, and \(y\) is the number of Pikachus in our encounter. 

If \(w \in \mathbb{N}\), \(b \in \mathbb{N}\), and \(N \in {0, ..., a + b}\), then for \(y \in {\text{max}(0, N - b), ..., min(w, N)}\),
\[
\text{Hypergeometric}(y | N, w, b) = \frac{\binom{w}{y} \binom{b}{N - y}}{\binom{w + b}{N}}
\]
...
Multivariate generalization of Binomial distribution. 

When categories is 2, becomes Binomial distribution.

When categories greater than 2 but the number of trials = 1, called **categorical distribution**.

Used when modeling unordered categorical response.



## Multivariate normal distribution

There is also a multivariate \(t\)
